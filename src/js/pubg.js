...
async function load() {
    ...
    $('.items-count').text(`Total Items: ${pubg.length}`);

    // 상품 정렬

    sortedNewest = Array.from(pubg).sort(function (a, b) {
        return b['id'] - a['id']
    })

    sortedHighestPrice = Array.from(pubg).sort(function (a, b) {
        return b['price'] - a['price']
    })

    sortedLowestPrice = Array.from(pubg).sort(function (a, b) {
