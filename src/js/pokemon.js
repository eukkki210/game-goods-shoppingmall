...
async function load() {
    ...
    $('.items-count').text(`Total Items: ${pokemon.length}`);

    // 상품 정렬

    sortedNewest = Array.from(pokemon).sort(function (a, b) {
        return b['id'] - a['id']
    })

    sortedHighestPrice = Array.from(pokemon).sort(function (a, b) {
        return b['price'] - a['price']
    })

    sortedLowestPrice = Array.from(pokemon).sort(function (a, b) {
