...
async function load() {
    ...
    sortedNewest = Array.from(objJson).sort(function (a, b) {
        return b['id'] - a['id']
    })

    sortedHighestPrice = Array.from(objJson).sort(function (a, b) {
        return b['price'] - a['price']
    })

    sortedLowestPrice = Array.from(objJson).sort(function (a, b) {
    ...
    if (objJson.length > 20) {
        let numberOfPages = Math.ceil(objJson.length / 20);
