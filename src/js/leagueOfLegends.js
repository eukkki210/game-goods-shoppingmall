...
async function load() {
    ...
    $('.items-count').text(`Total Items: ${leagueOfLegends.length}`);

    // 상품 정렬

    sortedNewest = Array.from(leagueOfLegends).sort(function (a, b) {
        return b['id'] - a['id']
    })

    sortedHighestPrice = Array.from(leagueOfLegends).sort(function (a, b) {
        return b['price'] - a['price']
    })

    sortedLowestPrice = Array.from(leagueOfLegends).sort(function (a, b) {
    ...
    if (leagueOfLegends.length > 20) {
        let numberOfPages = Math.ceil(leagueOfLegends.length / 20);
