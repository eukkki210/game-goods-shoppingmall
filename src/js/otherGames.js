...
async function load() {
    ...
    $('.items-count').text(`Total Items: ${others.length}`);

    // 상품 정렬

    sortedNewest = Array.from(others).sort(function (a, b) {
        return b['id'] - a['id']
    })

    sortedHighestPrice = Array.from(others).sort(function (a, b) {
        return b['price'] - a['price']
    })

    sortedLowestPrice = Array.from(others).sort(function (a, b) {
