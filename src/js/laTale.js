...
async function load() {
    ...
    $('.items-count').text(`Total Items: ${laTale.length}`);

    // 상품 정렬

    sortedNewest = Array.from(laTale).sort(function (a, b) {
        return b['id'] - a['id']
    })

    sortedHighestPrice = Array.from(laTale).sort(function (a, b) {
        return b['price'] - a['price']
    })

    sortedLowestPrice = Array.from(laTale).sort(function (a, b) {
