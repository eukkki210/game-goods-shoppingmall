...
async function load() {
    ...
    $('.items-count').text(`Total Items: ${overWatch.length}`);

    // 상품 정렬

    sortedNewest = Array.from(overWatch).sort(function (a, b) {
        return b['id'] - a['id']
    })

    sortedHighestPrice = Array.from(overWatch).sort(function (a, b) {
        return b['price'] - a['price']
    })

    sortedLowestPrice = Array.from(overWatch).sort(function (a, b) {
